...
libname cal "Z:\output_calendar_sas\cal" ;
 ...
 /******************************************************************************/
  ...
  DATA &out;
   ...
   DO k=1 TO (Lastyear-Firstyear+1)*12;
    ...
    deaster=MDY(mois,jour,year);     /* Dimanche de Pâques */
    EasterG=PUT(deaster,DATE9.);
    jascen=deaster+39;               /* Jeudi de l'Ascension */
    lpent=deaster+50;                /* Lundi de Pentecôte */
    ...
    /* Jeudi de l'Ascension; 39 jours après le dimanche de Pâques */
    ...
    /* Lundi de Pentecôte; 11 jours après l'ascencion; 50 jours après Pâques */
    ...
    /* Fête du Travail ; Premier Mai mais fêtée depuis 1947 seulement */
    a=WEEKDAY(MDY(5,1,year));
    IF ((month=5) AND (year>=1947)) THEN Off(a)=Off(a)+1;

    /* Armistice de 1945 ; Huit Mai mais fêtée depuis 1982 seulement */
    ...
    /* Armistice de 1918 ; fêtée depuis 1922 */
