%macro INITIALISE_CJO(chemin_macros_cjo=,stockage_cjo=,andeb=,anfin=,periodicite=);
 ...
 /* Attention : les dates de début et de fin sont indiquées en dur car on veut être sur d'avoir les mêmes régresseurs CJO pour tous les
...
/*mensuel*/
%INITIALISE_CJO(chemin_macros_cjo=Z:\,stockage_cjo=Z:\output_calendar_sas,andeb=,anfin=,periodicite=m);
libname demipont "Z:\output_calendar_sas\regresseurs";
data demipont.reg_JO;
set Tab_regs;
run;
proc export data=demipont.reg_JO
outfile="Z:\output_calendar_sas\regresseurs\reg_cjo_m_tmp.xls"
DBMS=XLS replace;
run;
