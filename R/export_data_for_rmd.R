...
reduce_calendar <- function(start = 0, end = 399) {
    if (start > end) stop("Erreur de date")
    return(calendar_easter |> dplyr::filter(year >= start & year <= end))
}
...
summarise_by_easter <- function(x, freq = 12, name = "aha") {
    if (freq == 12) {
        x <- x |> dplyr::summarise(e = sum(Freq), 
...
calendar_other_holidays <- create_empty_calendar(start = 1970, end = 2399, 
                                                 starting_day = "jeudi") |> 
    add_new_year() |> 
    add_may_day() |> 
    add_victory_day() |> 
