...
mean_rjd <- crossing(month_number = 1:12, weekday_number = 1:7) |> 
    dplyr::mutate(Day_mean = rep(length_mois / 7, each = 7), 
                  Off_mean = 0, 
                  In_mean = 0, 
...
mean_rjd <- mean_rjd |> 
    dplyr::mutate(Off_mean = dplyr::case_when(
...
mean_rjd[mean_rjd$month_number %in% 3:4 & mean_rjd$weekday_number == 2, "Off_mean"] <- mean_easter_monday + mean_rjd[mean_rjd$month_number %in% 3:4 & mean_rjd$weekday_number == 2, "Off_mean"]
mean_rjd[mean_rjd$month_number %in% 5:6 & mean_rjd$weekday_number == 2, "Off_mean"] <- mean_whit_monday + mean_rjd[mean_rjd$month_number %in% 5:6 & mean_rjd$weekday_number == 2, "Off_mean"]
