...
export_cjo_rjd <- function(name = "reg_cjo_m_rjd.csv", 
    ...
    reg1 <- rjd3modelling::htd(
        calendar = frenchCalendar, frequency = 12, start = start_reg, length = length_reg, 
    ...
    colnames(jeu_reg) <- c(colnames(reg1), 
                           colnames(reg2), 
                           colnames(reg3), 
                           colnames(reg5), 
                           colnames(reg6))
    
    jeu_reg_df <- data.frame(date = zoo::as.Date(time(jeu_reg)), 
                             jeu_reg)
    
    write.csv(jeu_reg_df, file = paste0("./output/", name), row.names = FALSE, quote = FALSE)
}
